...
public class CodeGeneratorTest {
  ...
  private static Program getGgtProgram(int a, int b) {
    ...
    Statement ggtWhile = new While(
        new Comparison(new Variable("b"), Comp.NotEquals, new Number(0)),
    ...
    Function ggt = new Function(new Int(), "ggt",
        new SingleDeclaration[]{new SingleDeclaration(new Int(), "a"),
            new SingleDeclaration(new Int(), "b")},
        new Declaration[]{new Declaration(new Int(), new String[]{"temp"})},
        new Statement[]{ggtSwap, ggtWhile, new Return(new Variable("a"))});
    Function mainFunctionGgt = new Function(new Int(), "main", new SingleDeclaration[]{},
        new Declaration[]{}, new Statement[]{
        new Return(
            new Call("ggt", new Expression[]{new Number(a), new Number(b)}))});
    Program ggtProgram = new Program(new Function[]{ggt, mainFunctionGgt}, null);
    return ggtProgram;
  }

  private static Program getFakProgram(int n) {
    Statement fakRecEnd = new IfThen(
        new Comparison(new Variable("n"), Comp.Equals, new Number(0)),
    ...
    Function fakFunc = new Function(new Int(), "fak",
        new SingleDeclaration[]{new SingleDeclaration(new Int(), "n")}, new Declaration[]{},
        new Statement[]{fakRecEnd, fakRec});
    Function mainFunctionFak =
        new Function(new Int(), "main", new SingleDeclaration[]{}, new Declaration[]{},
            new Statement[]{new Return(new Call("fak", new Expression[]{new Number(n)}))});
    Program fakProgram = new Program(new Function[]{mainFunctionFak, fakFunc}, null);
    return fakProgram;
  }
